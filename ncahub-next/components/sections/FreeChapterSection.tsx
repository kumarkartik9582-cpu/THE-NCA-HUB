    ...
    <section className="sec" id="sample" aria-label="Get free chapter" ref={ref}
      ...
      <div className="w">
        <div style={{ maxWidth: 640 }}>
          ...
          {/* Step flow with AnimatePresence — smooth wipe between steps */}
          <motion.div
            ...
            <AnimatePresence mode="wait">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {SUBJECTS.map((s, i) => (
                      <motion.button
                        ...
                        transition={{ duration: 0.4, delay: 0.5 + i * 0.07, ease: EXPO }}
                        whileHover={{ x: 6, borderColor: 'rgba(201,168,76,.4)', background: 'rgba(201,168,76,.09)' }}
                        whileTap={{ scale: 0.98 }}
                        onClick={() => { setSubject(s.label); setStep('email') }}
                        style={{
                          ...
                          borderRadius: 3, transition: 'background .2s, border-color .2s',
                        }}
                      >
                        {s.label}
                      </motion.button>
              ...
              {step === 'email' && (
                ...
                >
                  <motion.div
                    ...
                    transition={{ duration: 0.4, delay: 0.1, ease: EXPO }}
                    style={{ padding: '10px 14px', background: 'rgba(201,168,76,.05)', borderLeft: '2px solid var(--g1)', marginBottom: 20 }}
                  >
              ...
              {step === 'done' && (
                <motion.div
                  ...
                  style={{ padding: '28px', background: 'rgba(201,168,76,.06)', border: '1px solid rgba(201,168,76,.2)' }}
                >
                  <motion.p
                    initial={{ opacity: 0, y: 12 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.15, duration: 0.5, ease: EXPO }}
                    style={{ fontFamily: 'var(--fd)', fontSize: '1.3rem', color: 'var(--g0)', marginBottom: 10 }}
                  >
                    Chapter sent ✓
                  </motion.p>
                  <motion.p
                    ...
                    style={{ fontSize: 'var(--sm)', color: 'var(--fog)' }}
                  >
                    Check your inbox for the free chapter from {subject}. Didn&apos;t arrive? Check spam, or email{' '}
                    <a href="mailto:thencahub@gmail.com" style={{ color: 'var(--g1)' }}>thencahub@gmail.com</a>.
                  </motion.p>
                </motion.div>
              )}
