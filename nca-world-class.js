...
/* ==============================================================
   NCA HUB — REGIONAL PRICING BANNER (geo-detected discount)
   Shows a dismissible discount card to visitors in target
   countries only. Detection uses /cdn-cgi/trace on our OWN
   domain (Cloudflare Pages), so no CORS and no dependency.
   Fail-safe: any timeout/error, no country, or a non-target
   country shows NO banner and full price. Codes below MUST
   match the live Payhip coupons exactly.
   ============================================================== */
(function(){
  'use strict';

  /* ---- EDIT THESE TWO LISTS TO CHANGE WHO SEES A DISCOUNT ---- */
  /* TIER 1 -> GLOBAL45 (45% off, brings $175 to ~$96) */
  var TIER1 = ['IN','PK','BD','NG','PH','KE','GH','LK','NP','ID','EG','ZW','UG','TZ','ET'];
  /* TIER 2 -> REGION25 (25% off, brings $175 to ~$131) */
  var TIER2 = ['ZA','JM','TT','MY','MX','TH','VN','BR','CO'];
  /* Everyone else (CA, US, GB, AU, EU, AE, etc.) -> no banner, full price */
  var OFFERS = { T1:{code:'GLOBAL45', pct:'45'}, T2:{code:'REGION25', pct:'25'} };
  /* ----------------------------------------------------------- */
  ...
  function detect(){
    ...
    fetch('/cdn-cgi/trace', ctrl ? { signal: ctrl.signal } : {})
  ...
  function render(offer){
    if (document.getElementById('ncah-region-offer')) return;
    var css = "#ncah-region-offer{position:fixed;bottom:24px;right:24px;width:320px;max-width:calc(100vw - 48px);background:#0a0a0f;border:1px solid rgba(201,168,76,.35);border-radius:14px;padding:18px 20px 20px;box-shadow:0 20px 50px rgba(0,0,0,.6),0 0 0 1px rgba(201,168,76,.06) inset;z-index:8850;font-family:'Bricolage Grotesque','Helvetica Neue',sans-serif;color:#EDE5CE;opacity:0;transform:translateY(14px);transition:opacity .35s ease,transform .35s cubic-bezier(.16,1,.3,1);}"
      + "#ncah-region-offer.ro-in{opacity:1;transform:translateY(0);}"
      + "#ncah-region-offer .ro-x{position:absolute;top:9px;right:11px;background:none;border:none;color:#6B6257;font-size:20px;line-height:1;cursor:pointer;padding:2px 6px;transition:color .2s;}"
      + "#ncah-region-offer .ro-x:hover{color:#C9A84C;}"
      + "#ncah-region-offer .ro-eyebrow{font-size:.56rem;letter-spacing:.24em;text-transform:uppercase;color:#C9A84C;font-weight:600;margin-bottom:10px;}"
      + "#ncah-region-offer .ro-text{font-size:.86rem;line-height:1.6;color:#998E7C;margin:0 0 14px;}"
      + "#ncah-region-offer .ro-text strong{color:#EDE5CE;font-weight:600;}"
      + "#ncah-region-offer .ro-coderow{display:flex;align-items:center;gap:10px;flex-wrap:wrap;}"
      + "#ncah-region-offer .ro-code{font-family:'Cormorant Garamond',Georgia,serif;font-size:1.15rem;letter-spacing:.08em;color:#F0D878;background:rgba(201,168,76,.1);border:1px dashed rgba(201,168,76,.4);border-radius:6px;padding:6px 14px;font-weight:600;-webkit-user-select:all;user-select:all;}"
      + "#ncah-region-offer .ro-copy{background:#C9A84C;color:#020204;border:none;border-radius:6px;padding:8px 16px;font-family:inherit;font-size:.68rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;cursor:pointer;transition:background .2s;}"
      + "#ncah-region-offer .ro-copy:hover{background:#F0D878;}"
      + "@media(max-width:500px){#ncah-region-offer{bottom:16px;right:12px;left:76px;width:auto;max-width:none;}}";
    var style = document.createElement('style');
    style.textContent = css;
    document.head.appendChild(style);

    var card = document.createElement('div');
    card.id = 'ncah-region-offer';
    card.setAttribute('role', 'complementary');
    card.setAttribute('aria-label', 'Regional pricing offer');
    card.innerHTML =
        '<button class="ro-x" type="button" aria-label="Dismiss">&times;</button>'
      + '<div class="ro-eyebrow">Regional pricing</div>'
      + '<p class="ro-text">Notes are priced for your region. Use this code for <strong>' + offer.pct + '% off</strong> at checkout.</p>'
      + '<div class="ro-coderow"><span class="ro-code">' + offer.code + '</span><button class="ro-copy" type="button">Copy code</button></div>';
    document.body.appendChild(card);
    requestAnimationFrame(function(){ card.classList.add('ro-in'); });

    card.querySelector('.ro-x').addEventListener('click', function(){
      card.classList.remove('ro-in');
      try { sessionStorage.setItem(DISMISS_KEY, '1'); } catch(e){}
      setTimeout(function(){ if (card) card.remove(); }, 320);
    });

    var copyBtn = card.querySelector('.ro-copy');
    copyBtn.addEventListener('click', function(){
      var ok = function(){ copyBtn.textContent = 'Copied'; setTimeout(function(){ copyBtn.textContent = 'Copy code'; }, 1800); };
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(offer.code).then(ok, function(){ selectCode(); });
      } else { selectCode(); }
    });
    function selectCode(){
      var el = card.querySelector('.ro-code');
      try { var r = document.createRange(); r.selectNodeContents(el); var s = window.getSelection(); s.removeAllRanges(); s.addRange(r); } catch(e){}
    }
  }

  function start(){
    if ('requestIdleCallback' in window) requestIdleCallback(detect, { timeout: 2500 });
    else setTimeout(detect, 1200);
  }
